import os, re, platform, shutil, subprocess, time, json, vdf, requests
...
def _release_year(ts):
    ...
    try:
        return str(datetime.utcfromtimestamp(int(ts)).year) if ts else ""
    except Exception:
        return ""
...
def get_non_steam_games():
    ...
    for key, game in shortcuts.items():
...
def set_shortcut_icon(unsigned_id, icon_path):
    ...
    try:
        ...
        changed = False
        for key, game in data.get("shortcuts", {}).items():
            aid = game.get("appid")
            if aid is not None and (aid & 0xFFFFFFFF) == unsigned_id:
                game["icon"] = icon_path; changed = True; break
        if changed:
...
def search_game(name, app_id=None):
    ...
    for q in dict.fromkeys(q for q in [clean, short] if q):
        try:
            d = requests.get(f"https://www.steamgriddb.com/api/v2/search/autocomplete/{requests.utils.quote(q)}",
                             headers=headers, timeout=5).json()
            if d["success"] and d["data"]:
                g = d["data"][0]; print(f"  Found: {g['name']} (ID: {g['id']})"); return g["id"]
        except (requests.exceptions.ConnectionError, requests.exceptions.Timeout):
            return False  # network error — caller should not skip permanently
        except Exception: pass
    print(f"  No results for: {clean}"); return None

def _pref_params(prefs, include_animated=True):
    params = {}
...
def get_artwork(sgdb_id, art_type, prefs=None):
    if prefs is None:
        prefs = load_prefs()
    key = load_api_key()
    headers = {"Authorization": f"Bearer {key}"}
    ...
    def fetch(params):
        try:
            d = requests.get(url, headers=headers, params=params, timeout=5).json()
...
def download_all_artwork(sgdb_id, unsigned_id, prefs=None, progress_cb=None):
    if prefs is None:
        prefs = load_prefs()
    _LABELS = {"grids": "Cover", "heroes": "Hero/Background", "logos": "Logo", "icons": "Icon"}
    art_types = {"grids": f"{unsigned_id}p", "heroes": f"{unsigned_id}_hero",
                 "logos": f"{unsigned_id}_logo", "icons": f"{unsigned_id}_icon"}
    backup_artwork(unsigned_id)
    os.makedirs(CACHE_FOLDER, exist_ok=True)
    results = {}
    # 4 art types × (1 main + 3 previews) = 16 steps max
    total_steps = len(art_types) * 4
    step = 0
    for art_type, base in art_types.items():
        art_label = _LABELS.get(art_type, art_type)
        images = get_artwork(sgdb_id, art_type, prefs)
        if not images:
            step += 4
            results[art_type] = None
            continue
        top_url = images[0]["url"]
        ext = top_url.split(".")[-1].split("?")[0]
        save_path = os.path.join(GRID_FOLDER, f"{base}.{ext}")
        step += 1
        if progress_cb: progress_cb(f"Downloading {art_label}", step, total_steps)
        download_artwork(top_url, save_path)
        if art_type == "icons": set_shortcut_icon(unsigned_id, save_path)
        thumbs = []
        ...
        results[art_type] = {"applied_url": top_url, "applied_path": save_path,
                              "option_urls": [img["url"] for img in images[:5]],
                              "option_meta": [
                                  {
                                      "animated": img.get("mime", "") in ("image/gif", "image/webp"),
