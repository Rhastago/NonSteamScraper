...
# ---------------------------------------------------------------------------
# _thumb_cache_path — content-addressed preview cache key
# ---------------------------------------------------------------------------

def test_thumb_cache_path_keys_on_image_id(tmp_path, monkeypatch):
    monkeypatch.setattr(fg, "CACHE_FOLDER", str(tmp_path))
    img = {"id": 12345, "url": "https://cdn/full.png", "thumb": "https://cdn/thumb.png"}
    path, key = fg._thumb_cache_path(img, img["thumb"])
    assert key == "id12345"
    assert os.path.dirname(path) == str(tmp_path)
    assert os.path.basename(path) == "thumb_id12345.png"


def test_thumb_cache_path_id_is_stable_regardless_of_url(tmp_path, monkeypatch):
    # Same image id -> same cache file even if the thumb url differs, so the key
    # is strictly the image identity (never the slot/fetch order).
    monkeypatch.setattr(fg, "CACHE_FOLDER", str(tmp_path))
    p1, _ = fg._thumb_cache_path({"id": 7}, "https://cdn/a.png")
    p2, _ = fg._thumb_cache_path({"id": 7}, "https://cdn/b.png")
    # Extension follows the url, but the key (and thus identity) is the same.
    assert os.path.basename(p1).startswith("thumb_id7.")
    assert os.path.basename(p2).startswith("thumb_id7.")


def test_thumb_cache_path_falls_back_to_url_hash_without_id(tmp_path, monkeypatch):
    monkeypatch.setattr(fg, "CACHE_FOLDER", str(tmp_path))
    url = "https://cdn/thumb.jpg"
    path, key = fg._thumb_cache_path({}, url)
    assert key.startswith("h")
    assert os.path.basename(path) == f"thumb_{key}.jpg"
    # Deterministic: same url -> same key.
    _, key2 = fg._thumb_cache_path({}, url)
    assert key == key2
    # Different url -> different key (distinct images never collide).
    _, key3 = fg._thumb_cache_path({}, "https://cdn/other.jpg")
    assert key != key3


def test_thumb_cache_path_strips_query_string_and_defaults_bad_ext(tmp_path, monkeypatch):
    monkeypatch.setattr(fg, "CACHE_FOLDER", str(tmp_path))
    # Query string after the extension must not pollute the saved extension.
    path, _ = fg._thumb_cache_path({"id": 1}, "https://cdn/img.webp?token=abc")
    assert os.path.basename(path) == "thumb_id1.webp"
    # A url with no usable extension defaults to .png rather than a junk suffix.
    path2, _ = fg._thumb_cache_path({"id": 2}, "https://cdn/noext")
    assert os.path.basename(path2) == "thumb_id2.png"
